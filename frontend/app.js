...
function buildDevices() {
  ...
  const devices = state.assets.map((asset) => {
    ...
    return {kind:"asset",assetId:asset.id,endpointId:endpoint?.id || null,name:asset.name,inventoryNumber:asset.inventory_number,hostname:endpoint?.hostname || null,building:asset.building,floor:asset.floor,room:asset.room,organization:asset.organization,endpoint,status:deviceStatus(endpoint)};
  });
...
async function detail(assetId, scroll = true) {
  try {
    ...
    $("show-asset-qr").onclick = async () => { try { const blob = await apiBlob(`/admin/assets/${asset.id}/qr.svg`); if (state.assetQrUrl) URL.revokeObjectURL(state.assetQrUrl); state.assetQrUrl = URL.createObjectURL(blob); const popup = window.open("", "assetguard-qr", "width=480,height=560"); if (!popup) { showToast("Разрешите всплывающее окно для QR-кода", true); return; } popup.document.write(`<title>AssetGuard QR</title><main style="font-family:system-ui;text-align:center;padding:24px"><h1>${escapeHtml(asset.name)}</h1><p>${escapeHtml(asset.inventory_number)}</p><img style="width:320px;height:320px" src="${state.assetQrUrl}" alt="QR"><p>Отсканируйте код, чтобы открыть карточку устройства.</p></main>`); popup.document.close(); } catch (error) { showToast(error.message, true); } };
