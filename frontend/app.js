...
function renderPhysicalIncident(incident, roomId) {
  ...
  const decision=latest?`<p class="physical-decision"><strong>${escapeHtml(physicalActionLabels[latest.action]||latest.action)}</strong> · ${escapeHtml(latest.actor)}<br>${escapeHtml(latest.comment)}</p>`:"";
...
$("physical-incident-form").addEventListener("submit",async(event)=>{event.preventDefault();const incidentId=state.physicalIncidentId,roomId=state.roomWorkspace?.room.id;if(!incidentId||!roomId)return;const button=$("physical-incident-submit"),body={action:$("physical-incident-action-select").value,comment:$("physical-incident-comment").value.trim()};button.disabled=true;try{await api(`/admin/locations/physical-incidents/${incidentId}/decision`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});await openRoomWorkspace(roomId);state.roomTab="incidents";renderRoomTab();$("physical-incident-dialog").close();showToast(body.action==="INVESTIGATE"?"Инцидент взят на проверку":"Решение по инциденту сохранено");}catch(error){showToast(error.message,true);}finally{button.disabled=false;}});
