...
class AssetRecord(Base):
    ...
    id: Mapped[UUID]=mapped_column(PostgreSQLUUID(as_uuid=True), primary_key=True, default=uuid4); organization_id: Mapped[UUID]=mapped_column(ForeignKey("organizations.id")); inventory_number: Mapped[str]=mapped_column(String(128)); name: Mapped[str]=mapped_column(String(255)); asset_type: Mapped[str]=mapped_column(String(16)); status: Mapped[str]=mapped_column(String(32)); building: Mapped[str|None]=mapped_column(String(255)); floor: Mapped[str|None]=mapped_column(String(64)); room: Mapped[str|None]=mapped_column(String(255)); notes: Mapped[str|None]=mapped_column(Text); created_at: Mapped[datetime]=mapped_column(DateTime(timezone=True)); updated_at: Mapped[datetime]=mapped_column(DateTime(timezone=True))
