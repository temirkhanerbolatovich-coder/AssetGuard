...
## Импорт сканированных PDF на Windows

Распознаватель OCR бесплатный: Tesseract OCR распространяется под Apache-2.0. Если сканы нужно проверять локально, установите Tesseract и языковые модели `rus`, `kaz`, `eng`. Настройте в локальном `.env`:
...
Перезапустите API после изменения `.env`. В Windows пути к каталогу моделей и временной папке Tesseract должны быть ASCII, без пробелов и доступными на запись: OCR передаёт имена временных файлов внешнему процессу. Проверьте языки командой `tesseract --tessdata-dir C:\AssetGuard\tessdata --list-langs`; ожидаются `eng`, `kaz`, `rus`. Не копируйте `.env`, исходные ведомости, OCR-картинки или модели из `.local` в Git.

В Dashboard откройте **Импорт и экспорт → Загрузить PDF**. Предпросмотр охватывает все строки: по 25 на странице, с поиском и флажком каждой позиции. Сопоставление выполняется по инвентарному номеру; строка показывает, будет создана новая запись или обновлена существующая. Для сканов процент OCR — ориентир качества распознанного текста, а не гарантия точности: сверьте каждую нужную позицию с оригиналом и снимите флажок у ошибочных строк. До нажатия **Импортировать выбранные** данные в реестр не записываются.

Импорт принимает PDF до 10 MiB; OCR обрабатывает не более 30 страниц. Пропускаются пустые формы и документы без распознаваемых отдельных инвентарных строк. Детали, лимиты и диагностика описаны в [руководстве PDF/OCR](pdf-import-ocr.md).
