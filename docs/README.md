...
Для нескольких Windows-компьютеров используйте мастер `AssetGuard-Agent-Setup-0.1.2.exe`, а не копирование рабочей папки проекта. Он запрашивает уникальные учётные данные устройства, устанавливает GLPI Agent как службу с автозапуском и включает только аппаратный профиль сбора. Сборка EXE, ограничения и проверка первого подключения описаны в [Windows operations](../scripts/windows/README.md#графический-установщик-для-других-компьютеров).
