...
- Vision показывает последнее annotated image, историю и сравнение baseline. Сводная карточка кабинета объединяет физическую сверку, цифровые данные Agent, инциденты и общую хронологию; запуск новой проверки пока выполняется в основном разделе Vision.
